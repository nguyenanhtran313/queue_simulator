...
| `Test_VSF_dinhgia_bds/` | Case study định giá bất động sản — **không liên quan** tới 2 thư mục VSF ở trên dù trùng tiền tố tên. `generate_data.py` sinh `real_estate_valuation_dataset.csv`, `compare_models.py` so sánh các model. | `python generate_data.py` rồi `python compare_models.py` |
| `Basic ML/` | Chuỗi bài tự học ML (case study dự đoán khách hàng ngân hàng phản hồi khuyến mãi — xem `_REQUIREMENT.md`), **đánh số thứ tự và phải chạy tuần tự** vì bài sau dùng output (`.pkl`, `.csv`) của bài trước: `01_EDA_and_Stats` → `02_KMeans_Segmentation` → `03_Logistic_Regression_Benchmark` → `04_XGBoost_Production`/`04b_LightGBM_Production` → `05_SHAP_Analysis` → `06_Expected_Profit_Calculation` → `07_Uplift_Modeling_ROI`. | `python 0X_Ten_Bai.py` theo đúng thứ tự |
